# v4.1 Installation Guide: Oracle Linux 9 VM (OCI)

This guide provides step-by-step instructions for deploying the Hindu Panchanga Converter v4.1 (Universal Context) on an Oracle Linux 9 VM in Oracle Cloud.
...
Clone the latest v4.1 code from the `cosmic` branch:
```bash
...
2. **Cosmic Visuals**: Scroll down to see:
   - "Celestial Perspective" (Ecliptic Chart)
   - "Cosmic Alignment" (Solar System Map)
...
- **Privacy (Stealth Mode)**: All images are served via **Base64 encoding** (no public URLs). Direct access to folders is blocked.
